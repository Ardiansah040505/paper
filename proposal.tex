...
Sebagai jawaban tentatif atas rumusan masalah, berikut adalah hipotesis yang akan diuji dalam peneltiian ini:
\begin{enumerate}
...
simulasi monte carlo dengan menggunakan \textit{software} Geant4, dimana simulasi fisika (penembakan dan pelakakan partikel foton dari sumber Cesium-137) dijalankan 
...
Data hasil pemindaian dari seluruh titk selanjutnya digabung menjad satu basis data (\textit{dataset}) yang digunakan sebaga i\textit{input} pada tahap evaluasi dan analisi. Dataset tersebut menjadi dasar dari proses normalisasi, perhitungan parameter, rekonstruksi untuk mengidentifikasi adanya 
keretakan pada pipa
...
Tahap awal analisis dilakukan melalui normalisasi jumlah cacahan terhadap jumlah partikel primer untuk menghilangkan pengaruh variasi statistk selama proses simulasi. Nilai cacahan yang dinormalikan dapat dihitung dengan menggunakan
\begin{equation}
...
Hasil rekonstruksi kemudian dievaluasi berdasarkan distribusi dar intensitas untuk mengidentifikasi dimana lokasi keretakan muncul. Posisi keretakan ditentukan melalui adanya intensitas maksimum (\textit{hotspot}) yang dirumuskan sebagai 
\begin{equation}
    I_{max} = max[I(x,y)]
\end{equation}
