...
\begin{titlepage}
    ...
    \noindent Proposal Skripsi oleh \namaMahasiswa\ ini telah diperiksa dan disetujui untuk diuji.\\[1cm]

    \begin{flushright}
    Malang, ..... Agustus 2026\\[1.5cm]
    \end{flushright}
...
Detektor yang digunakan untuk mendeteksi radiasi sinar gamma yang terpancar pada sumber radioaktif seperti Cs-137 memerlukan sistem detektor yang memiliki sensitivitas tinggi, kestabilan yang tinggi, serta kemampuan akuisisi data secara kontinyu \cite{Yu2024}\cite{Prihtiadi2021}. Detektor sintilator NaI(TI) banyak digunakan karena
memiliki \textit{light yield} sehingga cocok digunakan untuk pemantuan radiasi serta efisiensi konversi foton yang baik \cite{Adhikari2021}\cite{Hahn2025}. Sinyal yang dihasilkan dari NaI(TI) kemudian diperkuan dengan menggunakan SiPM (\textit{Silicon Photomultiplier Thube}) yang memiliki ukuran yang kompak dan stabilitas yang baik \cite{Lee2024}\cite{Carlin2025}. 
...
\begin{enumerate}
    \item Bagaimana karakteristk respon hamburan compton sinar gamma terhadap keberadaan \textit{crack} (keretakan) pada sistem pipa dengan Geant4?
...
Sebagai jawaban tentatif atas rumusan masalah, berikut adalah hipotesis yang akan diuji da;am peneltiian ini:
\begin{enumerate}
    ...
    \item Performa dari sistem detektor sangat bergantung pada konfigurasi geometri simulasi. Efisiensi dari detektsi akan menurun seiring dengan bertambahnya jarak antara detektor, sumber, dan pipa akibat atenuasi dan pelebaran ruang. Sebaliknya, performa dari detektor akan sangat optimal pada posisi tertentu (umumnya mendekati $180^\circ$)
...
\begin{enumerate}
    \item Menghasilkan rekomendasi berupa parameter yang optimal untuk alat inspeksi. Hal ini langsung meminimalkan risiki kegagalan dan menghemat pembuatan prototype
...
\begin{enumerate}
    \item Sumber gamma yang dimodelkan di Geant4 dibatasi hanya berasal dari peluruh Cs-137 dengan energi yang terpancar pada $662 KeV$
    \item Detektor yang dimodelkan pada Geant4 adalah detektor NaI(T) atau Sodium Iodide. Karakteristik penguatan sinyal diasumsikan merepresentasikan
...
\begin{enumerate}
    ...
    Geant4 (\textit{Geometry AND Tracking}) adalah toolkit dengan basis bahasa pemrograman C++ berorientasi objek (OOP) yang dikembangkan oleh CERN. Perangkat lunak ini dirancang khusus untuk memodelkan interaksi partikel dengan materi secara akurat.
    ...
    \begin{itemize}
        ...
        interaksi hamburan balik Compton.
...
tanpa menyebabkan terjadinya ionisasi. Dalam bidang teknik nuklir dan pengujian tak merusak (NDT/\textit{non destructive testing}), radiasi pengion seperti sinar gamma banyak digunakan karena memiliki \textit{penetration} atau daya tembus yang tinggi sehingga memberikan informasi mengenai bentuk struktur internal suatu material \cite{geist2024nondestructive,donner2025comparative}.
...
Dengan $I_0$ adalah intensitas awal, $I$ intensitas setelah melewati material, $\mu$ koefisien atenuasi linier, dan $x$ merupakan ketebalan material. Nilai koefisien atenuasi bergantung pada energi foton dan jenis dari material yang berinteraksi \cite{aloraini2022design}. Sehingga nilai atenuasi tersebut dapat digunakan untuk mengindentifikasi adanya perubahan struktur atau adanya cacat seperti korosi, retak, atau lubang \cite{alanazi2024evaluating}. Perubahan ini 
...
Pada penelitian ini karena menggunakan prinsip interaksi hamburan balik Compton, detektor radiasi harus mampu membedakan hamburan dari foton yang berasal dari berbagai posisi didalam material \cite{Yu2024}. Oleh karena itu, karakteristik detektor seperti efisiensi deteksi, resolusi energi, waktu respons, dan sensitivitas sangat menentukan kualitas data yang diperoleh \cite{Yu2024_arxiv}. Jenis detektor yang sering digunakan adalah detektor sintilator Sodium Iodide yang diaktivasi Talium NaI(TI) karena 
memiliki efisiensi deteksi yang tinggi pada sinar gamma, \textit{cost} yang rendah, serta ukuran dan dimensi kristal yang dapat dibuat lebih besar sehingga meningkatkan probabilitas adanya \textit{cross section} terhadap foton \cite{Prihtiadi2018}. Meskipun energi resolusi yang dimiliki NaI(TI) lebih kecil dibandingkan detektor semikonduktor yang lain seperti HPGe, NaI(TI) menjadi pilihan utama dalam berbagai aplikasi industri karena praktis dan ekonomis \cite{Yu2024}.
...
Pada penelitian ini menggunakan metode gamma compton \textit{backscatter} sebagai teknik inspeksi \textit{non-destruktif} untuk mendeteksi cacat pada pipa migas. Sistem inspeksi dimodelkan dengan Geant4 dengan sumber radiasi gamma Cesium-137 dan detektor NaI(TI). Informasi atau hasil output simulasi berupa distribusi jumlah foton hamburan kemudian dianalisis menggunakan metode rekonstruksi citra tomografi untuk menggambarkan posisid dan ukuran keretakan di dalam pipa. Pendekatan ini diharapkan mampu menghasilkan 
...
simulasi monte carlo dengan menggunakan \textit{software} Geant4, dimana simulasi fsika (penembakan dan pelakakan partikel foton dari sumber Cesium-137) dijalankan 
...
(KBK) Fisika Komputasi dan Teoriti, Departmene Fisika, Universitas Negeri Malang.
...
\begin{itemize}
    ...
    \item variabel Terikat  : Spektrum energi hamburan balik, jumlah cachan, fluk gamma, Nilai perbedaan relatif, citra tomograffi
...
Tahap awal penelitian dilakukan dengan melakukan proses \textit{bernchmarking} simulasi untuk memvalidasi model simulasi Geant4 daam merepresentasikan respon detektor terhadap
...
radioaktif Cesium-137 yang merupakan sumber radiasi gamma standart yang banyak dgunakan dalam kalibarasi spektrometer gamma.
...
\begin{equation}
    \sigma N = \sqrt{N}
\end{equation}
dengan N adalah jumlah cacahan yang dterima dari setiap \textit{channel}. Ketidakpastian relatif dari simulasi dapat dihitung dengan menggunakan persamaan
\begin{equation}
...
Secara numerik, energi hasil \textit{smearing} diperoleh dengan menambahkan bilangan acak yang mengikuti prinsip distribusi normal ke settiap energi hasil simulasi
\begin{equation}
    E_{smear} = E + \sigma Z
\end{equation}
...
distribusi gaussian, sedangankan $Z$ merupakan bilangan yang acak yang mengikuti distribusi normal standart $N(0,1)$. Persamaan ini digunakan pada proses setelah pemrosesan untuk 
...
membuat bentuk konfigurasi dari sistem inspeksi berbasis Gamma Compton \textit{Backscatter}, yang terdisi dari pipa, sumber radiasi gamma Cesium-137, dan detektor NaI(TI). Model diharapkan mampu 
menggambarkan kondisi inspeksi secara nyata sehingga proses transportasi partikel dapat disimulasikan secara akurat.

Objek yang disimulaskan berupa silinder berongga dengan menggunakan kelas \textit{G4Tubs}, dengan parameter berupa jar-jari dalam, jari-jari luar, serta panjang dari pipa. Material penyusun pipa 
...
Sumber gamma yang digunakan adalah radionujklida Cesium-137 yang memancarkan energi sebesar $661,657 KeV$. Posisi sumber berada diluar pipa dengan orientasi tegak lurus terhadap permukaan pipa sehingga 
foton dapat dipancarkan dan berinteraks dengan pipa menghasilkan hamburan. Konfigurasi ini dipilih karena dapat mendeteksi adanya hamburan balik karena detektor dan sumber ada di sisi yang sama.

Sistem detektor terdiri atas enam buah detektor NaI(TI) yang disusu secara melingkar (mengelilingi sumber radiasi) yang ditunjukkan oleh Gambar \ref{fig:geometri_pipa}. Setiap detektor dimodelkan sebagai silinder menggunakan 
...
Selama proses simulasi, foton gamma yang dpancarkan oleh Cesium-137 ditransportaskan melalu material piap dengan model fisika elektromagnetka pada Geant4. Pada rentang energi 662 KeV interaksi yang dominan terjadi adalah hamburan compton 
mengikuti persamaan 
\begin{equation}
    E' = \frac{E}{1 + \frac{E}{m_e c^2}(1 - \cos\theta)}
\end{equation}
...
\end{equation}
dengan $E_{dep}$ adalah total energi yang terdeteksi oleh detekor dan $\Delta E_i$ adalah yang ditransfer pada setiap proses interaksi. Selain energi deposisi, jumlah foton yang terdeteksi juga akan dicatat untuk setiap detektor dan dinyatakan sebagai 
\begin{equation}
    N = \sum_{i=1}^{n} 1
\end{equation}
...
sumber radiasi Cs-137 dan enam detektor NaI(TI) yang bergerak secara bersamaan sebaga satu modul pada sepanjang area inspeksi. Konfigurasi tersebut menerapkan prinsip \textit{Gamma Compton Backsactter}, sehingg seluruh pemindaian dilakukan hanya dar satu sisi pipa saja.

Pemindauan dilakukan dengan menggunakan koordinat kartesiusn dengan membagi area inspeksi menjadi banyak ttik pengamatan (\textit{scan points}). Posisi sumber dan detektor pada seiap titik dinyatakan dengan persamaan \
\begin{equation}
    P_i = (x_i, y_i)
\end{equation}
Dengan $x_i$ dan $y_i$ merupakan koordinat titik pemindauan ke-i. Jumlah keseluruh titik pemindaian dihitung menggunakan
\begin{equation}
    N_s = N_x \times N_y
\end{equation}
dengan $N_p$ merupakan jumlah partkel primer pada setiap titik pemindaian 

Selama proses simulasi berlangsung, setiap detektor merekan deposisi energi serta jumlah cacahan foton hamburan yang diterima oleh detektor sensitif. informasi tersebut kemudian disimpan sebagai \textit{output} simulasi yang berisi, koordinat pemindaian, \textit{detector id}, jumlah 
partikel primer, deposisi energi dan jumlah cacagan foton untuk setiap titik pengukuran.

Data hasil pemindaian dari seluruh titk selanjutnya digabung menjad satu basis data (\textit{dataset}) yang digunakan sebagau \textit{input} pada tahap evaluasi dan analisi. Dataset tersebut menjadi dasar dari proses normalisasi, perhitungan parameter, rekonstruksi untuk mengidentifikasi adanya 
keretakan pada pipa
...
\textit{software} ROOT CERN sehingga diperoleh parameter kuantitatif yang digunakan untuk mengevaluasi respon sistem deteks terhadap keberadaan keretakan pada pipa.

Tahap awal analisis dilakuakn melalui normalisasi jumlah cacahan terhadap jumlah partikel primer untuk menghilangkan pengaruh variasi statistk selama proses simulasi. Nilai cacahan yang dinormalikan dapat dihitung dengan menggunakan
\begin{equation}
...
Karena data hasil pemindaian diperoleh dalam bentuk titik-titik diskrit, dilakukan dengan interpolasi spasial untuk membentuk distribusi data kontinu pada bidang yang diamati. Selanjutnya menerapkan Gaussian filter untuk mengurangi fluktuasi statistik akibat adanya eror pada simulasi Monte Carlo 
...
Citra tomografi kmeudian direkonstruksi menggunakan metode \textit{backprojection}, yaitu dengan menjumlahkan kontribusi nilai \textit{Relative Difference} dari seluruh posisi \textit{scanning} terhadap piksel citra. intensitas dari rekonstruksi dinyatakan sebagai
\begin{equation}
...
Hasil rekonstruks kemudian devaluasi berdasarkan distribusi dar intensitas untuk mengidentifikasi dimana okasi keretakan muncul. Posisi keretakan ditentukan melalui adanya intensitas maksimum (\textit{hotspot}) yang dirumuskan sebagai 
\begin{equation}
    I_{max} = max[I(x,y)]
\end{equation}
